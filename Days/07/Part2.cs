namespace _07
{
    internal class Part2
    {
        static public bool Solve(ulong result, int index, ulong expected, ulong[] numbers)
        {
            ...
            return Solve(result + numbers[index], index + 1, expected, numbers) 
                || Solve(result * numbers[index], index + 1, expected, numbers) 
                || Solve(ulong.Parse(result.ToString() + numbers[index].ToString()), index + 1, expected, numbers);
        }
